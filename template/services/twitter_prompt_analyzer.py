...
class TwitterPromptAnalyzer:
    ...
    async def fetch_by_prompt_analysis(self, prompt, prompt_analysis):
        prompt_analyzer_url = os.environ.get("TWITTER_PROMPT_ANALYZER_URL")

        res = requests.post(
            prompt_analyzer_url,
            json={"prompt": prompt, "prompt_analysis": prompt_analysis.dict()},
        )
    ...
    async def analyse_prompt_and_fetch_tweets(self, prompt):
        try:
            ...
            response = await self.fetch_by_prompt_analysis(
                prompt=prompt, prompt_analysis=prompt_analysis
            )
    ...
    async def retry_with_fixed_query(
        ...
        for attempt in range(retry_attempts):
            try:
                ...
                self.set_max_results(prompt_analysis.api_params)
                result = await self.fetch_by_prompt_analysis(
                    prompt=prompt, prompt_analysis=prompt_analysis
                )
